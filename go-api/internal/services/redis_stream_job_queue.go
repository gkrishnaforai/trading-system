...
const (
	...
	JobTypeTradingDecisionV3  JobType = "trading_decision_v3"
)
...
type TradingDecisionV3JobPayload struct {
	RunID       string `json:"run_id"`
	PortfolioID string `json:"portfolio_id"`
	Symbol      string `json:"symbol"`
...
func (q *RedisStreamJobQueue) EnqueueTradingDecisionV3Job(ctx context.Context, payload TradingDecisionV3JobPayload) (string, error) {
	if payload.Attempt <= 0 {
		payload.Attempt = 1
	}
	if payload.MaxAttempts <= 0 {
	...
	id, err := q.client.XAdd(ctx, &redis.XAddArgs{
		...
		Values: map[string]any{
			"job_id":      jobID,
			"job_type":    string(JobTypeTradingDecisionV3),
			"payload":     string(b),
			"enqueued_at": time.Now().UTC().Format(time.RFC3339Nano),
		},
	}).Result()
