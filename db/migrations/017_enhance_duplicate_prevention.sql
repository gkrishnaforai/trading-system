...
ALTER TABLE raw_market_data ADD COLUMN IF NOT EXISTS updated_at TIMESTAMP;
ALTER TABLE aggregated_indicators ADD COLUMN IF NOT EXISTS updated_at TIMESTAMP;

-- Update existing rows to set updated_at to current timestamp
-- SQLite: Use datetime('now') for current timestamp
UPDATE raw_market_data SET updated_at = CURRENT_TIMESTAMP WHERE updated_at IS NULL;
UPDATE aggregated_indicators SET updated_at = CURRENT_TIMESTAMP WHERE updated_at IS NULL;
...
ALTER TABLE raw_market_data ADD COLUMN IF NOT EXISTS data_source TEXT;
ALTER TABLE raw_market_data ADD COLUMN IF NOT EXISTS data_frequency TEXT CHECK(data_frequency IN ('intraday', 'daily', 'weekly', 'monthly', 'quarterly', 'yearly'));
...
ALTER TABLE aggregated_indicators ADD COLUMN IF NOT EXISTS data_source TEXT;
ALTER TABLE aggregated_indicators ADD COLUMN IF NOT EXISTS data_frequency TEXT CHECK(data_frequency IN ('intraday', 'daily', 'weekly', 'monthly', 'quarterly', 'yearly'));
