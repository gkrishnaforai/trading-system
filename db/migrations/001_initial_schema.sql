-- PostgreSQL-compatible initial schema for AI Trading System
-- Replaces SQLite AUTOINCREMENT with SERIAL/BIGSERIAL
...
-- Raw Market Data table (PostgreSQL version)
CREATE TABLE IF NOT EXISTS raw_market_data (
    ...
    fundamental_data JSONB, -- JSON snapshot of fundamental data
    options_data JSONB, -- JSON snapshot of options data
    news_metadata JSONB, -- JSON with headline, date, sentiment_score
    created_at TIMESTAMP DEFAULT CURRENT_TIMESTAMP,
    UNIQUE(stock_symbol, date)
);

-- Aggregated Indicators table (PostgreSQL version)
