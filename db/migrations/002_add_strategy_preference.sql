...
ALTER TABLE users ADD COLUMN IF NOT EXISTS preferred_strategy TEXT DEFAULT 'technical' CHECK(preferred_strategy IN ('technical', 'hybrid_llm', 'custom'));

-- Add strategy preference to portfolios (allows per-portfolio strategy)
ALTER TABLE portfolios ADD COLUMN IF NOT EXISTS strategy_name TEXT DEFAULT NULL;
