#!/usr/bin/env python3
"""
Test DRY Signal Calculator with 2025 Data
Validates the core logic with real historical data
"""
...
def test_2025_data():
    ...
    print("🎯 Testing DRY Signal Calculator with 2025 Data")
    print("=" * 60)
    ...
    # Test specific dates
    test_dates = [
        "2025-03-17",  # Should be BUY with aggressive logic
        "2025-05-01",  # Should be BUY with aggressive logic  
        "2025-07-25",  # Known BUY signal
        "2025-08-04",  # Known BUY signal
        "2025-10-15",  # Should be SELL
    ]
    ...
    for test_date in test_dates:
        ...
        date_mask = df['date'] == pd.to_datetime(test_date).date()
        matching_rows = df[date_mask]
        ...
        try:
            conditions = calculate_market_conditions(df, index)
            result = calculator.calculate_signal(conditions, symbol="TQQQ")
            
            print(f"{test_date}: {result.signal.value:4} ({result.confidence:.2f}) - {result.reasoning[0][:60]}...")
            
        except Exception as e:
            print(f"{test_date}: ❌ Error - {e}")
    
    # Test signal distribution across all 2025 data
    print(f"\n📊 2025 Signal Distribution Analysis:")
    print("-" * 40)
    ...
    print(f"  DRY System BUY Rate:    {buy_rate:.1f}%")
    print(f"  Improvement:            {buy_rate/4.9:.1f}x")
    
    print(f"\n✅ 2025 Data Testing Complete!")
    print(f"  • DRY architecture working with real data")
