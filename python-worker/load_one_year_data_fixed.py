...
def load_one_year_historical_data():
    ...
    try:
        ...
        for date, row in tqqq_data.iterrows():
            cursor.execute(
                "INSERT INTO raw_market_data_daily (symbol, date, open, high, low, close, volume, adjusted_close, data_source, created_at) VALUES (%s, %s, %s, %s, %s, %s, %s, %s, %s)",
                ('TQQQ', date.date(), 
        ...
        for date, row in df.iterrows():
            if not pd.isna(row['sma_20']) and not pd.isna(row['sma_50']):
                cursor.execute(
                    "INSERT INTO indicators_daily (symbol, date, sma_20, sma_50, sma_200, ema_20, rsi_14, macd, macd_signal, macd_hist, atr, bb_width, created_at, updated_at, data_source) VALUES (%s, %s, %s, %s, %s, %s, %s, %s, %s, %s, %s, %s, %s)",
                    ('TQQQ', date.date(), 
