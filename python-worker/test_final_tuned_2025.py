#!/usr/bin/env python3
"""
Final Tuned DRY Signal Calculator for 30-40% BUY Rate Target
Precisely calibrated configuration for production deployment
"""
...
class FinalTunedSignalCalculator:
    """Final tuned signal calculator for 30-40% BUY rate target"""
    ...
    def calculate_signal(self, conditions: MarketConditions, 
        ...
        # Apply symbol-specific adjustments (precisely tuned)
        ...
        # Precisely tuned signal logic for 30-40% target
        if is_oversold and is_recently_down:
            ...
            # Oversold in uptrend (strong signal)
            signal = SignalType.BUY
            confidence = 0.6
            reasoning.extend([
                "Oversold in uptrend",
        ...
        elif is_moderately_oversold and is_uptrend and macd_bullish:
            # Moderately oversold with strong confirmation
            signal = SignalType.BUY
            confidence = 0.5
            reasoning.extend([
                "Moderately oversold with confirmation",
                f"RSI moderately oversold: {conditions.rsi:.1f}",
                "Uptrend + MACD bullish",
                "Strong technical setup"
            ])
        elif is_mildly_oversold and is_uptrend and macd_bullish and not is_recently_down:
            # Mildly oversold with multiple confirmations
            signal = SignalType.BUY
            confidence = 0.4
            reasoning.extend([
                "Mildly oversold with multiple confirmations",
                f"RSI mildly oversold: {conditions.rsi:.1f}",
                "Uptrend + MACD bullish + no recent decline",
                "Conservative entry"
            ])
        ...
        elif is_uptrend and macd_bullish and not is_overbought and conditions.rsi < 60 and not is_mildly_oversold:
        ...
        if signal == SignalType.BUY and conditions.rsi < config.rsi_oversold:
            confidence = min(0.8, confidence + config.oversold_boost)
        
        if signal == SignalType.BUY and is_uptrend:
            confidence = min(0.8, confidence + config.trend_boost)
    ...
    def _apply_symbol_adjustments(self, symbol: Optional[str]) -> SignalConfig:
        ...
        if symbol == "TQQQ":
            # Precisely tuned TQQQ adjustments for 30-40% target
            config.rsi_oversold = 45  # More conservative - only very oversold
            config.rsi_moderately_oversold = 38  # Conservative
            config.rsi_mildly_oversold = 42  # Conservative
            config.max_volatility = 8.5  # Moderate volatility tolerance
            config.oversold_boost = 0.1  # Standard boost
            config.trend_boost = 0.08  # Slightly lower trend boost
...
def test_final_tuned_2025_data():
    """Test final tuned DRY signal calculator with 2025 data"""
    
    print("🎯 Testing Final Tuned DRY Signal Calculator with 2025 Data")
    print("=" * 60)
    ...
    # Initialize final tuned calculator
    calculator = FinalTunedSignalCalculator()
    
    # Test signal distribution across all 2025 data
    print(f"\n📊 Final Tuned 2025 Signal Distribution Analysis:")
    print("-" * 50)
    ...
    print(f"  Final Tuned BUY Rate:    {buy_rate:.1f}%")
    print(f"  Improvement vs Current:  {buy_rate/4.9:.1f}x")
    
    # Configuration summary
    print(f"\n⚙️ Final Tuned Configuration:")
    print("-" * 50)
    print(f"  TQQQ RSI Oversold: 45 (conservative)")
    print(f"  TQQQ RSI Moderate: 38 (conservative)")
    print(f"  TQQQ RSI Mild: 42 (conservative)")
    print(f"  TQQQ Max Volatility: 8.5% (moderate)")
    print(f"  Confidence Cap: 0.8 (moderate)")
    print(f"  Selective BUY conditions with confirmation")
    ...
    if deployment_ready:
        ...
        print(f"  ✅ Deploy to trading system")
    ...
    print(f"  • Target Rate: {'✅' if deployment_ready else '⚠️'} {buy_rate:.1f}% BUY signals")
    print(f"  • Risk Management: ✅ Volatility filtering")
    print(f"  • Symbol-Specific: ✅ TQQQ optimized")
    print(f"  • Maintainable: ✅ Centralized logic")
    
    print(f"\n✅ Final Tuned 2025 Data Testing Complete!")

if __name__ == "__main__":
    test_final_tuned_2025_data()
