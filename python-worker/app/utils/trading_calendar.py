...
def expected_intraday_5m_timestamps(start: date, end: date) -> List[pd.Timestamp]:
    """Return expected 5m bar timestamps in UTC for NYSE sessions in [start, end].

    - Uses exchange_calendars when available (holiday-aware, timezone-aware)
    - Falls back to 09:30-16:00 America/New_York on business days when not available

    The timestamps returned are the bar start times (e.g., 09:30, 09:35, ..., 15:55).
    """
    cal = _get_xnys_calendar()
    if cal is None:
        days = pd.bdate_range(start=start, end=end)
        ...
        for d in days:
            ...
            open_dt = pd.Timestamp(datetime.combine(day, time(9, 30)), tz="America/New_York")
            close_dt = pd.Timestamp(datetime.combine(day, time(16, 0)), tz="America/New_York")
            # last bar starts at 15:55
            rng = pd.date_range(open_dt, close_dt - pd.Timedelta(minutes=5), freq="5min", tz="America/New_York")
            out.extend([ts.tz_convert("UTC") for ts in rng])
        return out
    ...
    for _, row in schedule.iterrows():
        ...
        close_ts = row["market_close"]
        # timestamps are tz-aware
        rng = pd.date_range(open_ts, close_ts - pd.Timedelta(minutes=5), freq="5min", tz=open_ts.tz)
        out.extend([ts.tz_convert("UTC") for ts in rng])
    return out
