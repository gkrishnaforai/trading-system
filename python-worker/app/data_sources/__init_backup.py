...
def _is_source_available(source_name: str) -> bool:
    ...
    if source_name == "massive":
        ...
        # FMP requires API key, but allow placeholder if it's configured as primary
        api_key = settings.fmp_api_key or ""
        api_key = api_key.strip()
        if not api_key or api_key in ["your_fmp_api_key_here", ""]:
            # Log warning but still consider it available if configured as primary
            if settings.primary_data_provider == "fmp" or settings.default_data_provider == "fmp":
                import logging
                logger = logging.getLogger(__name__)
                logger.warning("⚠️ FMP is configured as primary but API key is not set. Please set FMP_API_KEY in .env")
                return True
            return False
        return True
    return True
